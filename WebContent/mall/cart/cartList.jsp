...
.d1 .s2 , .d1 .s3 { display:inline-block; width: 120px; padding: 6px 17px; border: 1px solid #333; font-size: 0.8em; border-radius:15px; font-weight: bold;text-align: center;}
...
/* 상단 d2 */
.d2 { width: 40%; padding: 15px 40px; margin: 5px 20px; float: left; display: inline-block; text-align: left; }
...
/* 상품 정보 테이블 */
.t_cart_list { width: 90%; border: 1px solid gray; border-collapse: collapse; margin: 0 auto; font-size:0.9em; text-align: center;
...
.td0 { text-align: center; font-weight: bold; font-size: 1.1em; padding: 10px 0;}
...
.td3 .s4, .td3 .s5 { font-weight: bold; font-weight: bold; color: #c84557;}
...
border: #196ab3; clear: both; background: #e6f0fa;}
.t_cart_tot tr {height: 100px;}
.t_cart_tot td, .t_cart_tot th { }
.t_cart_tot td, .t_cart_list th { border-top: 1px solid #dee2e6; border-bottom: 1px solid #dee2e6; border-left:none; border-right:none;}
...
.t_cart_tot tr:nth-child(2) { font-size: 0.6em; text-align: left;}
.t_cart_tot tr:nth-child(2) span { font-weight: normal;}
.t_cart_tot tr:nth-child(2) span b { color: #a13b66;}
.t_cart_tot tr:nth-child(2) th{ border-top: 1px solid #196ab3; text-align: center;}
...
.t_cart_address input[type=button] { width: 90px; height: 30px; background: #fff; border: 1px solid lightgray; border-radius: 3px;
padding: 5px; font-size: 0.9em;}

/* 하단 - 주문, 쇼핑 계속 버튼 */
.d5 { width: 90%; padding: 15px; margin: 10px 45px; text-align: center;}
.d5 input[type=button] { width: 150px; height: 50px; font-size: 1.0em; font-weight: bold; color: #fff; cursor: pointer; border-radius: 5px;}
.d5 #btn_buy_select3 { background: #3a85c8; border: 1px solid #3a85c8; margin-right: 5px;}
...
<script>
	document.addEventListener("DOMContentLoaded", function() {
		...
		let cart_ids = document.getElementsByName("cart_id"); // 카트 아이디의 배열
		...
		// 전체 선택 체크 박스 처리
		let cart_ids_list = []; // 체크 박스 선택 시 선택 되는 카트 아이디를 저장하는 배열
		let ck_count = 0; // 각 상품별 체크 박스 개수
		...
		ck_cart_all.addEventListener("change", function() {
			...
			k_count = 0;
			p_count = 0;
			if (ck_cart_all.checked) { 			// 전체 선택을 체크했을 때 -> 하위 모든 체크박스 선택
			...
			} else { 							// 전체 선택을 해제했을 때 -> 하위 모든 체크박스 해제
				ck_count = 0;
				cart_ids_list = [];
				for(let i = 0; i < ck_cart_ones.length; i++ ){
			...
			cart_ids_list = [...new Set(cart_ids_list)]; // 중복 카트 아이디 제거
		...
		// 각 상품별 체크 박스 중에서 해제된 것이 있다면 전체 선택 체크 박스를 해제
		// 각 상품별 체크 박스가 모두 체크 전체 선택 체크 박스를 선택
		for(let i = 0; i < ck_cart_ones.length; i++ ){
			ck_cart_ones[i].addEventListener("change", function(event) {
				if (ck_cart_ones[i].checked == false) {
					ck_cart_all.checked = false;
					--ck_count;
					cart_ids_list = cart_ids_list.filter((e) => e !== cart_ids[i].value); // 해제되지 않은 카트 아이디 다시 저장
					p_s1 -= parseInt(p_sums[i].value);
					p_s2 -= parseInt(p_sums2[i].value);
					p_s3 -= parseInt(p_sums3[i].value);
					--k_count;
...
String local = address.substring(0, 2); //주소에서 2글자만 추출 ex) 서울, 경기, 대구, 제주 ...
...
// 상품 가격(정가), 할인율, 할인 가격(판매가)
int product_price = 0;
int discount_rate = 0;
int buy_price = 0;
int buy_count = 0;
int p_sum = 0; // 각 상품의 합계(할인 가격)
int p_tot = 0; // 주문 상품의 총 합계(할인 가격)
int p_sum2 = 0; // 각 상품의 합계(원가)
int p_tot2 = 0; // 주문 상품의 총 합계(원가)

int d_count = 0; // 할인된 금액, ex) 10000원(원가), 10% 할인, 9000원(할인가), 1000원(할인 금액)
int p_sum3 = 0;	 // 각 상품의 할인된 금액의 합계
int p_tot3 = 0;  // 주문 상품의 할인된 급액의 총 합계

int k_count = 0; // 상품의 종류의 개수
int p_count = 0;	// 상품의 총 개수 
	...
	<div class="cart_list">
		...
		<table class="t_cart_list">
			...
			<%for(CartDTO cart : cartList) {
				...
				p_sum = buy_price * buy_count;	// 각 상품의 합계(할인 가격 * 주문 수량) 
				p_tot += p_sum;					// 총 합계(할인 가격의 총합계)
				
				p_sum2 = product_price * buy_count; // 각 상품의 원가 합계
				p_tot2 += p_sum2;
				
				p_sum3 = product_price * discount_rate/100 * buy_count; // 각 상품의 할인 금액 합계
				p_tot3 += p_sum3;
			...
			<form action="cartUpdatePro.jsp" method="post" name="cartForm">
				...
				<tr>
					...
					<td class="center td2" width="8%">
						<a href="../shopping/shopContent.jsp?product_id=<%=cart.getProduct_id()%>"><img src="/images_ezenmall/<%=cart.getProduct_image()%>" width="60" height="90"></a></td>
					<td class="left td3" width="45%">
						<span class="s1"><a href="../shopping/shopContent.jsp?product_id=<%=cart.getProduct_id()%>"><%=cart.getProduct_name() %></a></span><br>
						<span class="s2"><%=cart.getManufacturing_com() %></span><br>
					...
					<td class="right td5" width="10%"><%=df.format(p_sum) %>원</td>
					<td class="center td6" width="12%"><%=d_day %><br> 예정</td>
			...
			<%} }%>
			<tr>
				<td colspan="7">EZENMALL 배송 상품 총 금액: <b class="c1_s1">원</b> (+배송비 0원)</td>
			</tr>
		</table>
		<div class="d4">
			<span>선택한 상품</span>
		...
		<table class="t_cart_tot">
			<tr>
				...
				<th>최종 결제 금액<br><span class="s4 c3_s1">0원</span></th>
			</tr>
			<tr>
				<th colspan="7">
					<span>정가 <b class="c1_s2">0원</b>에서</span> <span>할인액 <b class="c1_s3">0원</b> 할인</span><br>
					<span>총 주문 수량: <b class="c1_s4">0종</b> (<b class="c1_s5">0개</b>)</span>
				</th>
			</tr>
		</table>
		<table class="t_cart_address">
			<tr>
				<th>배송일 안내<br><input type="button" value="배송 안내"></th>
				<td>배송지  : <%=address %> <input type="button" value="배송지 변경"></td>
			</tr>
		</table>
		<div class="d5">
			<input type="button" value="주문하기" id="btn_buy_select3">&ensp;&ensp;
			<input type="button" value="쇼핑 계속" id="btn_shopping">
		</div>
		<hr class="d_line2">
	</div>
